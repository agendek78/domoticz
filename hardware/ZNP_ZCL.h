...
class ZNP_ZCL
{
	...
	ZNPBase		*m_znp;
	boost::unordered_map<uint16_t, boost::shared_ptr<ZNP_ZCLNode> >	m_nodes;

public:
	ZNP_ZCL(ZNPBase*);

	void OnNewPacket(const ZNPPacket_t *packet);

	static void dumpBuffer(const uint8_t *data, uint8_t len);
};
